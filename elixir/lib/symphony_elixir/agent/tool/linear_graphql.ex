defmodule SymphonyElixir.Agent.Tool.LinearGraphql do
  @moduledoc """
  `linear_graphql` 工具的 backend-neutral 实现。
  """
  ...
  @description """
  Lower-level fallback for executing a raw GraphQL query or mutation against Linear using Symphony's configured auth. Prefer `linear_issue_read`, `linear_comment_create`, and `linear_issue_update_state` for common issue workflows. Some MCP clients may display or call this tool as the namespaced `symphony-linear_linear_graphql`; use that namespaced form when it is the visible tool name. Pass the GraphQL document in `query` and optional JSON object values in `variables`. Do not include Linear API tokens in the query, variables, files, logs, commits, or issue comments.
  """
  @input_schema %{
    "type" => "object",
