...
| HTTP server | MiMo/OpenCode 未实现；Omnigent HTTP backend 已第一阶段接入 | MiMo/OpenCode 不建议先做；Omnigent 可用 | 是，但实现和运维复杂度更高 |
...
5. 对 MiMo-Code/OpenCode，只有在 ACP 不能满足“远程共享 server / UI 复用 / 更完整 inspect API”时，再实现 HTTP server backend；Omnigent 已通过专用 `omnigent_http` backend 完成第一阶段接入。
...
长期：如果 MiMo-Code/OpenCode 需要服务化共享、远程 attach 或更丰富 UI/API，再实现对应 HTTP server backend。届时应把 server lifecycle、认证、端口、SSE 可靠性作为一等设计问题处理；Omnigent HTTP backend 的后续重点则是 managed host、runner 取消兜底和更完整的真实 smoke 覆盖。
