...
3. `direct_execution` 计划只在 registry 中落 root 节点，并让 root issue 作为普通 `issue` phase 执行；如果计划没有显式指定 `agent_id`，root issue 会继续使用普通 `routing` 规则选择 agent。
4. 派生 issue 只有在 registry 节点为 `ready` 且依赖已完成时才会执行。
5. 普通 issue agent 写出 `issue_result.json` 后，Symphony 写回 Issue Result 评论，并按 registry 依赖边解锁下游。
6. review issue agent 也写出 `issue_result.json`。`outcome=pass` 会把 review 节点标记为 `completed`，并把依赖满足的下游节点推进到 `ready`。
7. review `needs_rework` 会创建普通返工 issue，原节点在 registry 中标记为 `superseded`，下游改为等待返工节点通过。
8. review `needs_replan` 会把 root workflow 标记为 `replanning`，废弃未完成节点，并把这些被废弃的派生 issue 移到终态，避免旧路径继续被轮询；随后调度 root issue 重新进入 planning，planner 会收到重规划原因和被审查 issue。
9. root workflow 最终必须有 final review；planner 未创建时 controller 会补建 `final_review`。
