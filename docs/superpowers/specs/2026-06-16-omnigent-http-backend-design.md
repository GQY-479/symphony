...
- `host.mode`: 第一版支持 `external`，表示使用已经通过 `omnigent host`
  注册到 server 的机器。
- `host.host_id`: 第一阶段建议显式配置。设置时直接传给 Omnigent session create。
  真实本地 smoke 采用显式 host id，暂不做多 host 自动发现和选择。
...
2. 解析 host。第一阶段使用显式 `host.host_id`；多 host 自动发现和选择留到后续阶段。
...
如果配置的 host 不在线，backend 应通过 runner ready 等待和 session snapshot
清晰暴露失败原因，并提示用户运行 `omnigent host` 或修正 host id。第一版不自动启动
Omnigent server / host，也不在多个 online host 之间自动调度。
...
- 配置的 host 不在线或 runner 未就绪：startup failed，返回 session snapshot 并提示检查 `host.host_id` 与 `omnigent host`。
