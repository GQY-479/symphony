...
本设计聚焦 MiMo-Code。OpenCode 复用同一 backend 的工作先跳过，只保留为后续方向，不纳入本轮实现范围。
...
本轮不做 OpenCode。等 MiMo-Code 的 ACP backend 跑通后，再评估 OpenCode 是否能直接复用 `acp_stdio` 配置。如果能复用，只新增配置和 smoke test；如果 ACP 行为有差异，再补充 provider-specific quirk，而不是新增平行 backend。
